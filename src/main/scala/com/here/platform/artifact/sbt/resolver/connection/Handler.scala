...
/**
  * For normal SBT usage this is a dummy URLStreamHandler so that here+// URLs can be created without throwing a
...
final class Handler extends URLStreamHandler {
  def openConnection(url: URL): URLConnection = new ArtifactURLConnection(url)
}
